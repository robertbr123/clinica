        ...
        if(is_writable($caminho)) {
            ...
            if($_POST['versao'] == 'novo') {
                if(!empty($_POST['senha']) && !empty($_POST['fantasia']) && $_POST['senha'] == $_POST['resenha'] && strlen($_POST['senha']) >= 6 && $myerro === 0) {
                    ...
                    mysql_query("CREATE DATABASE IF NOT EXISTS ".$_POST['bd']) or die('Cria��o da base: '.mysql_error());
                    mysql_select_db($_POST['bd'], $conn) or die('Sele��o da base: '.mysql_error());
                    $sqls = extraiSQL('bases/bd_novo.sql');
                    foreach($sqls as $sql) {
                        mysql_query($sql) or die('Importa��o nova: '.mysql_error());
                    }
                    $sql = "UPDATE funcionarios SET senha = MD5('".$_POST['senha']."') WHERE cpf = '11111111111'";
                    mysql_query($sql) or die('Altera��o de senha: '.mysql_error());
                    ...
                    mysql_query($sql) or die('Altera��o de dados da cl�nica: '.mysql_error());
                    header('Location: ./');
                } else {
                ...
                if($myerro === 0) {
                    ...
                    foreach($sqls as $sql) {
                        mysql_query($sql) or die('Importa��o Atualiza��o: '.mysql_error().' - '.$sql);
                    }
                    $config = file($caminho);
                    $config[51] = "    \$server = '".$_POST['server']."';\n";
                    $config[52] = "    \$user = '".$_POST['user']."';\n";
